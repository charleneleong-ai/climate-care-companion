...
class RuleExporter:
    ...
    def write(self) -> tuple[Path, Path]:
        ...
        return RULES_PATH, CORPUS_PATH


def main() -> None:
    rules, corpus = RuleExporter(Corpus.load()).write()
    print(f"wrote {rules.relative_to(rules.parents[2])}")
    print(f"wrote {corpus.relative_to(corpus.parents[2])}")
